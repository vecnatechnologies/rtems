...
int can_bus_de_init_default( can_bus *bus )
{
  (void) bus;
  return -EIO;
}
...
int can_bus_get_num_filters_default( can_bus *bus )
{
  (void) bus;
  return -EIO;
}

int can_bus_set_flags_default(
  can_bus *bus,
  uint32_t flags
)
{
...
int can_bus_register( can_bus *bus )
{
  int               rv;
  rtems_status_code sc;
  char              bus_path[ 12 ];
