...
UART_HandleTypeDef UartHandles[ NUM_PROCESSOR_CONSOLE_UARTS +
  ...
  const UART_HandleTypeDef *huart )
{
  ...
  for ( i = 0UL; i < COUNTOF( stm32f_console_driver_table ); i++ ) {
    if ( stm32f_console_driver_table[ i ].base_driver_info.handle == huart ) {
      return (stm32f_base_uart_driver_entry *) &( stm32f_console_driver_table[
                                                    i ].base_driver_info );
    }
  }
  ...
  for ( i = 0UL; i < COUNTOF( stm32f_uart_driver_table ); i++ ) {
    if ( stm32f_uart_driver_table[ i ].base_driver_info.handle == huart ) {
      return (stm32f_base_uart_driver_entry *) &( stm32f_uart_driver_table[ i ]
                                                  .base_driver_info );
    }
  }
...
static void stm32f_init_uart_clock( const stm32f_uart Uart )
{
  switch ( Uart ) {
...
static void stm32f_uart_reset( const stm32f_uart Uart )
{
  switch ( Uart ) {
#if defined( STM32_ENABLE_USART_1 )
    case STM32F_UART1:
...
static void stm32f_init_gpio_clock( const stm32f_gpio_port port )
{
  switch ( port ) {
...
static GPIO_TypeDef *stm32f_get_gpio( const stm32f_gpio_port port )
{
...
static void stm32f_init_dma_clock( const stm32f_dma_controller controller )
{
...
static void stm32f_uart_dma_rx_isr( void *argData )
{
...
static void stm32f_uart_dma_tx_isr( void *argData )
{
  stm32f_base_uart_driver_entry *pUART =
    (stm32f_base_uart_driver_entry *) argData;
...
static void stm32f_uart_isr_termios( void *arg )
{
...
static void stm32f_uart_isr( void *arg )
{
...
static rtems_status_code stm32f_get_uart_isr(
...
)
{
  ...
  if ( stm32f_get_uart_driver_entry_from_handle( huart ) != NULL ) {
    ...
    *isr_routine = &stm32f_uart_isr_termios;
    ...
    stm32f_error_handler_with_reason(
      "UART handle is not a console or standard uart." );
...
int stm32f_register_interrupt_handlers( UART_HandleTypeDef *huart )
{
  ...
  stm32f_base_uart_driver_entry *pUart =
    stm32f_get_base_uart_driver_entry_from_handle( huart );
  ...
  if ( stm32f_get_uart_isr( huart,
         &uart_isr_to_install ) == RTEMS_SUCCESSFUL ) {
...
int stm32f_remove_interrupt_handlers( UART_HandleTypeDef *huart )
{
  ...
  if ( stm32f_get_uart_isr( huart,
         &uart_isr_to_remove ) == RTEMS_SUCCESSFUL ) {
...
void HAL_UART_MspDeInit( UART_HandleTypeDef *huart )
{
  ...
  HAL_GPIO_DeInit( stm32f_get_gpio( pUart->tx_pin.port ),
    ( 1 << pUart->tx_pin.pin ) );
  /* Configure UART Rx as alternate function  */
  HAL_GPIO_DeInit( stm32f_get_gpio( pUart->rx_pin.port ),
    ( 1 << pUart->rx_pin.pin ) );
...
static rtems_status_code stm32f_send_uart_event(
  UART_HandleTypeDef    *huart,
  const UARTCallbackType type
)
{
  rtems_status_code sc = RTEMS_UNSATISFIED;

  stm32f_uart_driver_entry *pUartDriver =
    stm32f_get_uart_driver_entry_from_handle( huart );
  ...
  if ( pUartDriver != NULL ) {
    switch ( type ) {
      case UARTCallbackType_TX:
        ...
        if ( ( pUartDriver->base_driver_info.handle->State ==
               HAL_UART_STATE_BUSY_RX ) ||
             ( pUartDriver->base_driver_info.handle->State ==
