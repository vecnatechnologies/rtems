...
static uint32_t num_ethernet_tx_dma_buffer_unavailable = 0UL;
...
uint32_t stm32f_ethernet_get_num_tx_dma_buffer_unavailable(void)
{
  return num_ethernet_tx_dma_buffer_unavailable;
}
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
  {
    ...
    {
      errval = ERR_USE;
      num_ethernet_tx_dma_buffer_unavailable++;
      goto error;
    }
